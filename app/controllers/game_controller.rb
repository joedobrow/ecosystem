class GameController < ApplicationController
  ...
  def generate_board
    ...
    (0...@board_width).each do |x|
      (0...@board_height).each do |y|
        ...
        create_unit(x, y, unit) unless unit.nil?
      end
    end
  end
  ...
  def create_egg(x, y)
    @game_state[:eggs].push({ x: x, y: y, health: @egg_health })
  end

  def move_dino_randomly(dino)
    directions = []
    if dino[:y] < (@board_height - 1)
      directions << :up
    end
    if dino[:y] > 0
      directions << :down
    end
    if dino[:x] < (@board_width - 1)
      directions << :right
    end
    if dino[:x] > 0
      directions << :left
  ...
  def poop_eggs
    @game_state[:dinos].combination(2).each do |dino1, dino2|
      ...
      if dino1[:x] == dino2[:x] && dino1[:y] == dino2[:y] && dino1[:health] > @dino_reproduce_health && dino2[:health] > @dino_reproduce_health
  ...
  def check_for_diseased_ferns
    for i in 1..(@board_width - 2)
      for j in 1..(@board_height - 2)
  ...
  def spawn_ferns
    for i in 0..@board_width
      for j in 0..@board_height
        if check_empty(i, j) && rand < @fern_spawnrate
  ...
  def adjust_fern_spawnrate
    @fern_spawnrate = @game_state[:ferns].length / (@board_width * @board_height)
  end
